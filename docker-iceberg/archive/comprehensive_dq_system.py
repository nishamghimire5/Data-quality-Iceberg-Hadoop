#!/usr/bin/env python3
"""
Comprehensive Data Quality System - Detailed Column Analysis
...
class ComprehensiveDQSystem:
            ...
            if non_null_count > 0:
                ...
                if 'int' in col_type.lower() or 'long' in col_type.lower() or 'double' in col_type.lower():
                    ...
                    try:
                        ...
                        # Check string length statistics                        lengths = df.select(length(col(col_name)).alias('len')).filter(col(col_name).isNotNull())
    ...
    def generate_html_report(self, all_analyses, timestamp):
        ...
        <body>
            ...
            <div class="summary">
                ...
                <ul>
                    <li><strong>Tables Analyzed:</strong> {len(all_analyses)}</li>
                    <li><strong>Total Records:</strong> {sum(a.get('total_rows', 0) for a in all_analyses):,}</li>
                    <li><strong>Total Columns:</strong> {sum(a.get('business_columns', 0) for a in all_analyses)}</li>
        ...
        for analysis in all_analyses:
            if 'error' in analysis:
                ...
                <div class="table-section">
                    ...
                    <p class="issues">Error: {analysis['error']}</p>
                </div>
                """
                continue
            ...
            for col in analysis.get('column_analyses', []):
                ...
                # Format range/values display
                range_display = ""
                if col.get('min_value') is not None and col.get('max_value') is not None:
                    range_display = f"{col['min_value']} - {col['max_value']}"
                elif col.get('most_frequent_values'):
                    top_values = [str(v['value'])[:20] for v in col['most_frequent_values'][:3]]
                    range_display = ", ".join(top_values)
                
                issues_display = "; ".join(col.get('issues', [])) if col.get('issues') else "None"
                ...
                html += f"""
                        <tr>
                            ...
                            <td>{range_display}</td>
                            <td class="quality-score {score_class}">{col['data_quality_score']}</td>
                            <td class="issues">{issues_display}</td>
                        </tr>
                """
    ...
    def generate_summary_report(self, all_analyses, timestamp):
        ...
        with open(summary_file, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("COMPREHENSIVE DATA QUALITY ANALYSIS - ALL 8 HOME CREDIT TABLES\n")
            f.write("=" * 80 + "\n")
            ...
            f.write("-" * 50 + "\n")
            f.write(f"Tables Analyzed: {len(all_analyses)}\n")
